...
| 项目 | 要求 |
| --- | --- |
| Device setup region / 设备初始化地区 | 和账号注册、长期运行目标环境一致 |
...
4. 发生封号或风控时，必须记录 Device setup region、系统语言、时区、IP 类型、出口地址、注册时间、首次登录时间和账号状态。
5. 这条经验不证明“Device setup region 选择中国 + IPv6”就是唯一原因；它是后续初始化必须检查的风险项。

记录模板：

```text
Device setup region / 设备初始化地区:
系统语言:
时区:
...
[ ] remotePort 已分配且唯一
[ ] 公网域名已分配
[ ] Device setup region、系统语言、时区已记录
[ ] 出口 IP 类型和出口质量已确认
