...
2. 安装基础软件
3. 检查 Device setup region 和网络出口风控
4. 准备目录
5. 拉取公开代码
6. 安装依赖和构建后台
...
## 4. Device setup region 和网络出口风控检查

N3 的经验需要写入新机器初始化标准流程：Windows 设备初始化阶段的 Device setup region 曾选择“中国”，同时当时网络出口使用 IPv6。后续注册 Claude 后，账号很快被封。

修正后的判断：IPv6 本身不是问题。如果 IPv6 出口足够干净、稳定，且注册、首次登录、长期运行环境一致，IPv6 也可以正常使用。真正需要关注的是出口质量、地区画像、Device setup region 和账号行为之间是否一致。

新机器上线前必须检查。当前标准：Device setup region 选择 Vietnam / 越南。

| 项目 | 要求 |
| --- | --- |
| Device setup region / 设备初始化地区 | 标准选择 Vietnam / 越南，并和账号注册、长期运行目标环境一致 |
...
| 变更记录 | 记录 Device setup region、出口 IP、注册时间、首次登录时间 |

操作原则：

1. Windows 设备初始化阶段的 Device setup region 标准选择 Vietnam / 越南。
2. 不要机械禁用 IPv6；干净 IPv6 可以使用。
3. 不要在注册后短时间内频繁切换地区、IP 类型、出口 ASN 或设备环境。
4. 如果 IPv6 出口质量不确定，先做账号注册和登录环境验证，再纳入生产节点。
5. 发生封号或风控时，必须记录 Device setup region、系统语言、时区、IP 类型、出口地址、注册时间、首次登录时间和账号状态。
6. 这条经验不证明“Device setup region 选择中国 + IPv6”就是唯一原因；它是后续初始化必须检查的风险项。

记录模板：

```text
Device setup region / 设备初始化地区: Vietnam / 越南
系统语言:
时区:
...
[ ] remotePort 已分配且唯一
[ ] 公网域名已分配
[ ] Device setup region 已选择 Vietnam / 越南，系统语言、时区已记录
[ ] 出口 IP 类型和出口质量已确认
...
忽略 Device setup region、时区、出口 IP 和账号注册环境一致性
